...
///////// ДОПОЛНЯЕМ ДВУМЕРНЫЙ МАССИВ МАРКА-МОДЕЛЬ-ЦЕНА СООТВЕТСТВУЮЩИМИ ИЗОБРАЖЕНИЯМИ И ДАТОЙ ЗАГРУЗКИ
         foreach($this->mas_file_name as $kef=>$masf) {  //%%%%% $this->mas_file_name!!!!!
            if(strpos(mb_strtoupper($masf), $mas12[$ke][0])!==false) {
               ...
               $mas12[$ke][]='<a href="#" style="position:absolute;z-index:1;" onclick="post_send('.$post_del_img.');return false;" >delete</a> <img src="img/'.$masf.'"> ';  //%%%%%!!!%%%%% ЗДЕСЬ (HTML), UNIX-КАТАЛОГИ (/home/localhost/www/img/) "НЕ ПОЙМЕТ"
               $mas12[$ke][]=date('d-m-Y');
            }
         }
/////////
...
//======================================================//%%%%%!!!%%%%%
if(isset($_FILES['userfile'])) {                             //%%%%%!!!%%%%%
$obj->Dir_rm_mk_copy('/home/khursa_3/www/img/');            //%%%%%!!!%%%%%
}                                                            //%%%%%!!!%%%%%

if (isset($_POST['dell_img'])) {
    $obj->Delete('/home/khursa_3/www/img/');
}

$obj->Dir_scan('/home/khursa_3/www/img/');                  //%%%%%!!!%%%%% ТУТ ВСЕГДА - НЕ ТОЛЬКО ПРИ if(isset($_FILES['userfile'])) {...
//======================================================//%%%%%!!!%%%%%
$obj->dat=$data;
$obj->Massiv();
$obj->Algoritm0();
